...
def obtain_user_id(user):
    try:
        response = requests.get(f'https://gitlab.com/api/v4/users?username={user}')
...
def obtain_profile_info(user_id):
    response = requests.get(f'https://gitlab.com/api/v4/users/{str(user_id)}')
    return response.json()


def obtain_status(user_id):
    response = requests.get(
        f'https://gitlab.com/api/v4/users/{str(user_id)}/status'
    )
...
def obtain_keys(user_id):
    response = requests.get(f'https://gitlab.com/api/v4/users/{str(user_id)}/keys')
    return response.json()


def obtain_gpg_keys(user_id):
    response = requests.get(
        f'https://gitlab.com/api/v4/users/{str(user_id)}/gpg_keys'
    )
...
def obtain_projects(user_id):
    response = requests.get(
        f'https://gitlab.com/api/v4/users/{str(user_id)}/projects'
    )
...
def obtain_project_info(project_id):
    response = requests.get(
        f'https://gitlab.com/api/v4/projects/{str(project_id)}/repository/commits/'
    )
